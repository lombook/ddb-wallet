...
@Transactional
@Slf4j
public class AccountTranfersService {
        ...
        if(accountZid == null){
            ...
            if(type.equals(1)){
                ...
                if(integralAccount.getAmount() < payAccAmount ) {
                    aliCloudMailsService.sendMail(payAccEmailAddress, "【大地宝】总账余额不足，请注意",remark+"：账户扣除不足 zid=" + accountZid );
                }
                //判断总账金额是否小于转账金额
                ...
                if(integralWallet.getAmount() < amount ) {
                    aliCloudMailsService.sendMail(payAccEmailAddress, "【大地宝】总账余额不足，请注意",remark+"：账户扣除不足 zid=" + integralWallet.getZid() );
